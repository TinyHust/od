...
angular.module("app").controller("DesignerController", function($scope, PatternService, ProductService, localStorageService, FontService, ArtService, QrcodeService, $interval, $upload, $timeout, LanguageService, AdminTemplateService) {
    ...
    $scope.setItemConfig = function(item, prop, value){
        if(!!item.configuration){
            ...
            if(config.hasOwnProperty(prop)){
                ...
                var con = {};
                con[prop] = value;
                item.configuration = _.meeger(config, con);
            }
        }else {
            var config = {};
            config[prop] = value;
    ...
    $scope.loadProduct = function(id, variation) {
        ProductService.getProduct(id, variation).then(function(data) {
            $scope.onLoadProduct = false;
            ...
            if (toType(data.design) == "object"){
                ...
                if(toType(data.fonts) === "array"){
                    var font_url = window.parent.nbds_frontend.font_url;
                ...
                cid = "c_" + product_id;
                if ((toType(localStorageService.get(sid)) === "object") && (OD_task == "") && !$scope.ignoreLocalDesign){
    ...
    $scope.loadLanguage = function(code) {
        ...
        LanguageService.getLang(code).then(function(data) {
            if(data.mes == "success"){
                ...
                langjs["LAYER_IMAGE"] = $scope.langs["IMAGE"] ? $scope.langs["IMAGE"] : "Image";
                ...
                langjs["LAYER_LINE"] = $scope.langs["LINE"] ? $scope.langs["LINE"] : "Line";
    ...
    $scope.getColorPath = function(paths) {
        ...
        _.each(paths, function(path, key) {
            if ((toType(path.fill) == 'string') && (path.fill.charAt(0) == "#")) {
                var color = path.fill;
            ...
            } else if (toType(path.fill) == 'string' && path.fill.charAt(0) == "r") {
    ...
    $scope.fitReferenceDesign = function(id){
        ...
        if((ref_width/ref_height) > (width/height)){
            ...
            var offset_top = (height - ref_height) * $scope.zoom * $scope.designScale / 2;
            $scope.canvas.forEachObject(function(obj) {  
                obj.top = obj.top + offset_top;
            })
        }else{
            $scope.resizeDesign(height/ref_height, 1);
            var offset_left = (width - ref_width) * $scope.zoom * $scope.designScale / 2;
            $scope.canvas.forEachObject(function(obj) {  
    ...
    $scope.saveDesign = function(orientation) {
        ...
        config['furl'] = window.parent.nbds_frontend.font_url;      
    ...
    $scope.updateCurrentLayerAfterDelete = function() {
        var index =
            $scope.canvas.getObjects().length;
    ...
    $scope.onFileSelect = function($files) {
        ...
        for (var i = 0; i < $files.length; i++) {
            ...
            if ($files[i].size > max_size * 1024 * 1024) {
                $scope.errorUpload = "Oops, file upload larger than " + window.parent.nbds_frontend.upload_max + " MB";
                continue
            }
...
angular.module("app").service("ProductService", function($http, $q) {
    this.getProduct = function(id, variation) {
        ...
        $http({
            url: window.parent.nbds_frontend.url,
            method: "POST",
            data: "action=nbdesigner_get_product_info&nonce=" + window.parent.nbds_frontend.nonce + "&id=" + product_id + "&oid=" + orderid + '&did=' + id + '&variation=' + variation + '&ref=' + OD_ref + '&folder=' + OD_folder,
            headers: {
...
var NBDESIGNERCART = {
    ...
    ajax_upload: function(data_img, data_json, flag) {
        var key = "frame_" + flag;
        var dataimg = {};
...
jQuery(document).ready(function($){
    $(window.parent.document).find('.variations_form.cart').on('change', function(){
        var variation_id = $(window.parent.document).find('input[name="variation_id"]').val();
        var scope = angular.element(document.getElementById("designer-controller")).scope();
        if(variation_id != ''){
        ...
        if(!scope.onLoadProduct && variation_id != ''){
            ...
            show_indicator();
            scope.canvas.clear();
            scope.loadProduct(product_id, variation_id);              
        }      
        if (scope.$root.$$phase !== "$apply" && scope.$root.$$phase !== "$digest") scope.$apply()
    });
    $(window.parent.document).find('.reset_variations').on('click', function(){
        var scope = angular.element(document.getElementById("designer-controller")).scope();
        window.parent.NBDESIGNERPRODUCT.nbdesigner_unready();
        if(!scope.onLoadProduct){
            ...
            show_indicator();
            scope.canvas.clear();
            scope.loadProduct(product_id, 0);              
        }      
        if (scope.$root.$$phase !== "$apply" && scope.$root.$$phase !== "$digest") scope.$apply()
    });    
});
