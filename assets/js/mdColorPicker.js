...
angular.module('mdColorPicker', [])
	.run(['$templateCache', function ($templateCache) {
		...
		var shapes = {
			...
			'plus': '<path d="M12,2c0.276,0,0.512,0.098,0.707,0.293S13,2.724,13,3v8h8c0.275,0,0.512,0.098,0.707,0.293S22,11.724,22,12s-0.098,0.512-0.293,0.707S21.275,13,21,13h-8v8c0,0.275-0.098,0.512-0.293,0.707S12.276,22,12,22s-0.512-0.098-0.707-0.293S11,21.275,11,21v-8H3c-0.276,0-0.512-0.098-0.707-0.293S2,12.276,2,12s0.098-0.512,0.293-0.707S2.724,11,3,11h8V3c0-0.276,0.098-0.512,0.293-0.707S11.724,2,12,2z"/>'
		};
		for (var i in shapes) {
	...
	.directive( 'mdColorPickerContainer', ['$compile','$timeout','$mdColorPalette','mdColorPickerHistory', function( $compile, $timeout, $mdColorPalette, colorHistory ) {
		return {
			...
			//	console.log( "mdColorPickerContainer Controller", Date.now() - dateClick, $scope );
				...
				$scope.defaults = $scope.defaults ? $scope.defaults : $scope.random ? tinycolor.random() : 'rgb(255,255,255)';
				...
				$scope.color = new tinycolor($scope.value || $scope.defaults); // Set initial color
...
.config(['$qProvider', '$mdThemingProvider', function ($qProvider, $mdThemingProvider){$qProvider.errorOnUnhandledRejections(false);}])
    .factory('$mdColorPicker', ['$q', '$mdDialog', 'mdColorPickerHistory', function ($q, $mdDialog, colorHistory) {
        ...
        return {
            show: function (options)
            {
				...
				var mdCallback = function(){return true;};
				...
				options.defaultValues = options.defaultValues === undefined ? '#FFFFF0' : options.defaultValues;
				...
				options.mdCallback = options.mdCallback === undefined ? mdCallback : options.mdCallback;				
                dialog = $mdDialog.show({
                                        parent:parentEl,
					templateUrl: 'mdColorPickerDialog.tpl.html',
					hasBackdrop: options.hasBackdrop,
					clickOutsideToClose: options.clickOutsideToClose,
					onRemoving:options.mdCallback,
					controller: ['$scope', 'options', function( $scope, options ) {
							...
							{
								$mdDialog.hide( $scope.value );
								colorHistory.add(new tinycolor($scope.value));
							...
							$scope.defaults = options.defaultValues;
